...
Date: 7/5/2025
Project description: CITS1401 project 2
...
def main(csvfile_1, csvfile_2):
    try:
        ...
        if 'Invalid data(header missing) in second file:' in cleaningdata_2(csvfile_2):
            print(cleaningdata_2(csvfile_2))
            return {},{},{}
        elif 'Invalid data(header missing): No age group' in cleaningdata_2(csvfile_2):
            print('Invalid data(header missing) in second file: No age group')
            return {},{},{}
        elif 'invalid dara(empty data)' in cleaningdata_2(csvfile_2):
            print('invalid dara(empty data) in second file')
            return {},{},{}
    
        if 'Invalid data(header missing) in first file:' in cleaningdata_1(csvfile_1):
            print(cleaningdata_1(csvfile_1))
            return {},{},{}
        
        if len(cleaningdata_2(csvfile_2)) <= 1 or len(cleaningdata_1(csvfile_1)) <= 1:
            print('Invalid data(empty file)')
            return {},{},{}
        
        
        OP1 = op1(csvfile_1,csvfile_2)
        OP2 = op2(csvfile_1,csvfile_2)
        OP3 = op3(csvfile_1,csvfile_2)
        #print(op1_1)
        #print(op2_1)
        #print(op3_1)
        
    except FileNotFoundError:
        print("Invalid file names or non-existent files, please try again")
        return {},{},{}
...
def cleaningdata_2(csvfile_2):
    ...
    if len(missing) > 0:
        return 'Invalid data(header missing) in second file:',missing
        ...
        if counts > 0:
            for r in rows[2:]:
                if r.isdecimal() == False:
                    ...
                    break 
        counts = counts + 1
    temper_list_removed_error_row_list = [Sample_Data_Populations_P2_list[0]]
    for rowss in Sample_Data_Populations_P2_list[1:]:
        if rowss not in error_number_list:
            temper_list_removed_error_row_list.append(rowss)
            
                
    Sample_Data_Populations_P2_list = temper_list_removed_error_row_list               
       
    if len(Sample_Data_Populations_P2_list) <=1:
        return 'invalid dara(empty data)'
...
#cleaningdata_2('SamplePopulations_tcase8.csv')
...
def cleaningdata_1(csvfile_1):
    ...
    for needed in hard_coded_title:
        if needed not in title_line:
            missing.append(needed)
    if len(missing) > 0:
        return'Invalid data(header missing) in first file:',missing
        #return 
    #end
...
#crosscheckfile_for_missing('SampleData_Areas_P2.csv','SamplePopulations_tcase7.csv')#SamplePopulations_tcase4#SampleData_Populations_P2_test
...
#     
    ...
    element_list = []
    index_list = []
    for group in reading_line_population:
        i = reading_line_population.index(group)
        index_list.append(i)
        group = group.split('-')
    ...
    #print(element_list)
    #print(index_list)
    # this is new, have not add into the final file for submission
    new_ele_list = []
    new_index_list = []
    for ele, i in zip(element_list,index_list):
        if len(ele) != 2:
            break
        intial = ''
        stepping = 0
        for k in ele:
            if k.isdecimal() or k == 'None':
                intial = intial + k +'-'
                stepping = stepping + 1
                if stepping == 2:
                    new_ele_list.append(intial[:len(intial)-1])
...
def op3(csvfile_1,csvfile_2):
    ...
    #handle when no SA3 satisfy the condition of sa3 with 15 or more sa2 code
    if len(mark_list) <= 0:
        return {}
...
"""
Issue 1 (Date 2025 May 11):
- Error Description:
    ZeroDivisionError: division by zero
- Erroneous Code Snippet:
    
    Variance =(1/length)*(sum_result) # line 448
    
- Test Case:
op2('SampleData_Areas_P2.csv','SamplePopulations_tcase3.csv')
    
- Reflection:
I understand that the length of the data could be one, and the formula that is used needs to be minus one,
so the length will be zero, leading to a zero division error. Hence, I put an if-else statement
to avoid such a case from happening. I check whether the length of the data is longer than one,
only if this is true, then I calculate the standard deviation. Otherwise, assign zero as required. 
...
- Erroneous Code Snippet:
    ...
    cosine_similarity = dot_product / (magnitudeA * magnitudeB) # line 1074
    
- Test Case:
main('SampleData_Areas_P2.csv','SamplePopulations_tcase1.csv')
    
- Reflection:
I realise that the magnitude A or B can go to Zero as the population that contains more than 15 SA2 codes
might contain Zero population group within because they only consider SA2 codes that have more than 15 groups.
Therefore, I implement a if,else statement to handle the case. The statement that handles and check
the situation when either magnitudeA or magnitudeB becomes zero, and assign the cosine similarity
with zero as required


Issue 3 (Date 2025 May 15):
- Error Description:
   ValueError: invalid literal for int() with base 10: ''
- Erroneous Code Snippet:
    
   Sample_Data_Populations_P2_list[n][2:] = map(int,Sample_Data_Populations_P2_list[n][2:]) # line 197
    
- Test Case:
main('SampleData_Areas_P2.csv','SamplePopulations_tcase8.csv')
    
- Reflection:
From this test case, I figured out that my program was not robust in handling invalid data with no population data.
So, I added lines to check whether the length of list that contain the population data is smaller than two or
not, since this situation suggest their is not valid population data. And I set it to return a message to
notice the user and end the program, because I cannot process any further.


Issue 4 (Date 2025 May 16):
- Error Description:
   skipping line using for loop with .remove
- Erroneous Code Snippet:
   for example: 
   for i in nested_list:
       if i != something:
           i.remove(i[-1])
This code structure will lead to skipping in element in a for loop
    
- Test Case:
main('SampleData_Areas_P2.csv','SamplePopulations_tcase8.csv')
    
- Reflection:
From this test case, the dynamic change of the element in a list will lead to skipping elements. This
issue will lead to misread, missing elements and allow elements that have not satisfied the if statement to remain
in the list. The more appropriate way will be to make another list that stores the error and remove it in another for loop.
To ensure the element is correct. 


Issue 5 (Date 2025 May 19):
- Error Description:
   IndexError: list index out of range
- Erroneous Code Snippet:
   
   marker = mark_list[0] # line 1065
    
- Test Case:
main('SampleData_Areas_P2.csv','Populations_Corner4.csv')
    
- Reflection:
From this test case, I have not considered whether SA3 does not satisfy the condition of being equal to or greater than 15 SA2.
To put the situation into my consideration, I use an if statement to verify the condition before proceeding
the calculation
